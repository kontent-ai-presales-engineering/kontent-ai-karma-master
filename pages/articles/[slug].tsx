...
import {
  ...
  getDefaultMetadata,
  getHomepage,
} from '../../lib/services/kontent-service';
...
const ArticlePage: FC<Props> = (props) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className={`py-1 px-3 w-full md:w-fit text-center`}>
          <h1
            className={`${
              mainColorTextClass[props.siteCodename]
            } mb-10 text-3xl tracking-wide font-semibold px-10`}
          >
            {article.elements.title.value}
          </h1>
        </div>
        <div className='bg-white rounded-lg opacity-80 p-4'>
      ...
      <div className='px-2 max-w-screen m-auto md:px-20'>
        ...
        <div className='flex flex-col gap-2'>
          <div className='w-fit p-2 bg-gray-800 text-white opacity-90 font-semibold rounded-lg px-4'>
            ...
            {article.elements.articleType.value.length > 0 &&
              article.elements.articleType.value.map((type) => (
                <div
                  ...
                  } rounded-lg px-4 text-sm font-semibold text-white`}
                >
                  {type.name}
                </div>
              ))}
