...
export const YouTubeMovieComponent: FC<Props> = props => {
  const params = new URLSearchParams({
    autoplay: isMultipleChoiceOptionPresent(props.item.elements.autoplay.value, 'yes') ? "1" : "0",
    start: props.item.elements.startTime.value ? props.item.elements.startTime.value.toString() : "0",
    end: props.item.elements.endTime.value ? props.item.elements.endTime.value.toString() : "9999"
  });
        ...
        <>
          ...
          <div className="row-span-3">
            <button className="relative flex justify-center items-center focus:outline-none focus-visible:ring focus-visible:ring-indigo-300 rounded-3xl group" onClick={() => { setModalOpen(true) }} aria-label="Watch the video">
              <Image className="rounded-3xl shadow-2xl transition-shadow duration-300 ease-in-out" src={thumb} width={thumbWidth} height={thumbHeight} priority alt={thumbAlt as string} />
              {/* Play icon */}
          ...
          <div className="text-4xl mt-5" {...createElementSmartLink(contentTypes.youtube_embed.elements.title.codename)}>{props.item.elements.title?.value}</div>
          <div className=""{...createElementSmartLink(contentTypes.youtube_embed.elements.body.codename)}><RichTextElement element={props.item.elements.body} isInsideTable={false} language={props.item.system.language} /></div>
