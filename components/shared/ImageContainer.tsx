...
export const ImageContainerComponent: FC<Props> = props => {
  ...
  const image = <Image
    ...
    width={thumbWidth}
    height={thumbHeight}
    priority alt={thumbAlt as string} />
  return (
    <div
    ...
    >
      {props.item.elements.imageLocation.value[0].codename == "left" &&
        image
      }
      <div className="px-6 py-4">
        <h5 className="mb-3 text-base font-semibold tracking-tight text-sky-600" {...createElementSmartLink(contentTypes.image_container.elements.title.codename)}>{props.item.elements.title?.value}</h5>
        <div className="mb-2 text-sm leading-normal text-justify text-sky-900" {...createElementSmartLink(contentTypes.image_container.elements.content.codename)}><RichTextElement element={props.item.elements.content} isInsideTable={false} language={props.item.system.language} /></div>
      </div>
      {props.item.elements.imageLocation.value[0].codename == "right" &&
        image
      }
    </div>
  );
