...
export const TranslationCustomElement: React.FC<Props> = ({ element, context, value, handleSave }) => {
    ...
    const [workflows, setWorkflows] = useState(new Array<WorkflowModels.WorkflowStep>())
    ...
    const loadStuckItems = async () => {
        ...
        const triggerWorkflow = workflows.find(wf => wf.id === savedValue?.triggerWorkflowId)
        ...
        if (triggerWorkflow) {
            ...
            const stuckItems = await Promise.all([...getStuckSources, ...getStuckTargets])
    ...
    const triggerSave = useCallback((savedValue: SavedValue) => {
    ...
    const handleTriggerWorkflowSelectionChange = (newTriggerId: string) => {
        let newReviewId = newTriggerId !== savedValue?.reviewWorkflowId ? savedValue?.reviewWorkflowId : ""
    ...
    const handleReviewWorkflowSelectionChange = (newReviewId: string) => {
        let newTriggerId = newReviewId !== savedValue?.triggerWorkflowId ? savedValue?.triggerWorkflowId : ""
    ...
    useEffect(() => {
        ...
        const getWorkflows = async () => {
            const workflowsResponse = await axios.get<Array<WorkflowModels.WorkflowStep>>("/api/get-workflows/")
    ...
    const resubmitItems = async (selectedOnly: boolean) => {
        ...
        const items = stuckItems.filter(si => !selectedOnly || selectedItems.includes(si.system.codename + si.system.language))
        const resubmits = items.map(i => {
            const languageId = languages.find(l => l.system.codename === i.system.language)?.system.id
            return axios.post("/api/translate-manual/", { itemId: i.system.id, languageId })
        })
    ...
    return <div className="custom-element p-1">
        <div className="container mx-auto">
            {context.variant?.id !== defaultLanguageId
                ? <>Only configurable via default language</>
                : <>
                    {/* <pre>{JSON.stringify(savedValue, null, 2)}</pre> */}
                    <h4 className="mb-2 mt-0 text-2xl font-medium leading-tight text-primary">Auto-translation Settings</h4>
                    <div className="flex flex-wrap  my-3">
                        <div className="relative flex-grow max-w-full flex-1 px-4">
                            <label className='block mb-2 text-sm font-medium text-gray-900 dark:text-white'>Source Language</label>
                            <select className='bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500'
                                value={savedValue?.sourceLanguageId ?? ""}
                                onChange={e => handleSourceLanguageChange(e.target.value)}>
                                <option value="">Select a language</option>
                                {languages.map(l => {
                                    const supportedLanguage = AzureTranslationService.getSupportedLanguageCode(l.system.codename)
                                    return <option key={l.system.id} value={l.system.id} disabled={supportedLanguage === null}>{l.system.name} ({supportedLanguage === null ? `language not supported` : `translated as ${supportedLanguage}`})</option>
                                })}
                            </select>
                        </div>
                    </div>
                    ...
                    <h4 className="mb-2 mt-0 text-2xl font-medium leading-tight text-primary">Languages to automatically translate</h4>
                    <ul className="flex flex-col pl-0 mb-0 border rounded border-gray-300  my-3">
                        {languages.map(language => {
                            const supportedLanguage = AzureTranslationService.getSupportedLanguageCode(language.system.codename)

                            if (language.system.id === savedValue?.sourceLanguageId) return null
                            return <li key={language.system.id} className="relative block py-3 px-6 -mb-px border border-r-0 border-l-0 border-gray-300 no-underline">
                                <div className="mb-[0.125rem] block min-h-[1.5rem] pl-[1.5rem]">
                                    <input className="absolute mt-1 -ml-6" type="checkbox" id={language.system.id} disabled={supportedLanguage === null} defaultChecked={savedValue?.targetLanguageIds?.includes(language.system.id)} onChange={() => handleLanguageSelectionChange(language.system.id)} />
                                    <label className="text-gray-700 pl-6 mb-0" htmlFor={language.system.id}>
                                        {language.system.name}  ({supportedLanguage === null ? `language not supported` : `translated as ${supportedLanguage}`})
                                    </label>
                                </div>
                            </li>
                        })}
                    ...
                    <h4 className="mb-2 mt-0 text-2xl font-medium leading-tight text-primary">Stuck Items</h4>
                    {stuckProcessing === "initial" ? "Click the load/refresh button to see any stuck items" : null}
                    {stuckProcessing === "loading" ? "Loading stuck items" : null}
                    {stuckProcessing === "resubmitting" ? "Resubmitting stuck items" : null}
                    {stuckProcessing === "loaded" && stuckItems.length === 0 ? "Nothing is stuck." : null}

                    {stuckItems && stuckProcessing !== "loading" && stuckProcessing !== "resubmitting" ? <ul className="flex flex-col pl-0 mb-0 border rounded border-gray-300  my-3">
                        {stuckItems.map(item => {
                            return <li key={item.system.id + item.system.language} className="relative block py-3 px-6 -mb-px border border-r-0 border-l-0 border-gray-300 no-underline">
                                <div className="relative block mb-2">
                                    <input className="absolute mt-1 -ml-6" type="checkbox" id={item.system.id + item.system.language} checked={selectedItems.includes(item.system.codename + item.system.language)} onChange={e => handleItemSelectionChange(item.system.codename + item.system.language)} />
                                    <label className="text-gray-700 pl-6 mb-0 w-full" htmlFor={item.system.id + item.system.language}>
                    ...
                    <div className="flex flex-wrap  my-3">
                        <div className="relative flex-grow max-w-full flex-1 px-4 flex">
                            <button className="inline-block align-middle text-center select-none border font-normal whitespace-no-wrap rounded py-1 px-3 leading-normal no-underline bg-blue-600 text-white hover:bg-blue-600 me-auto" onClick={() => loadStuckItems()}>Load/Refresh Items</button>
                            <button className="inline-block align-middle text-center select-none border font-normal whitespace-no-wrap rounded py-1 px-3 leading-normal no-underline bg-gray-600 text-white hover:bg-gray-700" onClick={() => resubmitItems(true)}>Re-Submit {selectedItems.length} selected items</button>
                            <button className="inline-block align-middle text-center select-none border font-normal whitespace-no-wrap rounded py-1 px-3 leading-normal no-underline bg-gray-600 text-white hover:bg-gray-700 ms-1" onClick={() => resubmitItems(false)}>Re-Submit all items</button>
                        </div>
                    </div>
                </>
            }
